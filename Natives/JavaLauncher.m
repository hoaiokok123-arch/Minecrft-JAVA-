...
static NSString* resolveLWJGLJarPath(NSString *librariesPath) {
    ...
    NSString *defaultPath = [librariesPath stringByAppendingPathComponent:@"lwjgl.jar"];
    if ([fm fileExistsAtPath:defaultPath]) {
        NSLog(@"[JavaLauncher] Requested LWJGL %@ not found. Falling back to 3.3.3.", selectedVersion);
        return defaultPath;
    }

    NSLog(@"[JavaLauncher] Warning: no LWJGL jar found in %@", librariesPath);
    return nil;
}
...
static NSString* buildLauncherClasspath(NSString *librariesPath, BOOL launchJar, NSString *launchJarPath) {
    ...
    for (NSString *file in allFiles) {
        ...
        [entries addObject:[librariesPath stringByAppendingPathComponent:file]];
    }

    if (launchJar && launchJarPath.length > 0) {
        [entries addObject:launchJarPath];
    }

    if (entries.count == 0) {
        return [NSString stringWithFormat:@"%@/*", librariesPath];
    }
